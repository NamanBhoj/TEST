...
def jsonify_face(csvface) ->dict:
                ...
                json_data = csv_face_data.to_json(r"W:\\Full_Application\\TEST\\Coordinates"+ '\\fileface_recorded.json')
                print(json_data)

def jsonify_centre(csvcenter) -> dict:
                """returns json from the csv file for the center csv"""
                csv_data = pd.read_csv(csvcenter)
              
                csv_data = (csv_data.drop('frameId',axis = 1))
                print(csv_data)
                json_data = csv_data.to_json(r"W:\\Full_Application\\TEST\\Coordinates" + '\\filecenter_recorded.json')
                print(json_data)
