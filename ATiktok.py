...
def crop_face(video,webcam_coordinate_data,smallvidname):
    ...
    if os.path.isdir('Coordinates') == False:
        os.system(f'mkdir Coordinates')  
    ...
    # print((values[0]))
    count1 = os.listdir(os.getcwd() + f'\{smallvidname}frames')
    print(len(count1))
...
#     #TODO: IF REMOVE THE -10 IN RANGE GET ERROR
    for i in range(1, iterate_times):
        ...
        im_0 = Image.open(os.getcwd() +f'\\{smallvidname}frames\{i}.jpg')
        

        # im1 = im.crop((xmin, ymin, xmax, ymax))
        im0 = im_0.crop((values[0][i], values[1][i], values[2][i], values[3][i]))
      
        im0.save(f'{smallvidname}frames_cropped\\{i}.jpg')
    ...
    count = os.listdir(os.getcwd() +f'\\{smallvidname}frames_cropped')
    print((count[-1]))
   ...
   #TODO: ADD THESE PATH DYNAMICALLY
        clip = ImageClip(os.getcwd() + f'\\{smallvidname}frames_cropped\\'  + a[i]).set_duration(1/fps_val)
        clips.append(clip)
        # print(i)
